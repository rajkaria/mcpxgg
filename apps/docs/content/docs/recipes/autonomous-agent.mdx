...
description: Give an agent a capped, revocable SpendingIntent budget and drive paid MCP calls until the owner pulls the plug.
---


This guide walks through giving an autonomous agent a **capped, revocable
budget** with a [`SpendingIntent`](/docs/concepts/intents), then driving paid MCP
...
the [Intent Manager](https://mcpx.gg/dashboard/intents) UI, or directly with a Move call:

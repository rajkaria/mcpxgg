---
title: Run your own marketplace
description: The full mcpx.config.json schema and publish lifecycle — describe, validate, publish a server.
---

The entire MCPX stack (gateway, facilitator, indexer, contracts, web) is
open. Running your own marketplace means pointing the components at your
own deployed Move package and operating the off-chain services. The
contract surface is documented in the [Move package reference](/docs/move/overview);
this recipe focuses on the developer-facing artifact every published
server needs — its `mcpx.config.json`.
